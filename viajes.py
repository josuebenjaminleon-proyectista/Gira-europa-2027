...
def renderizar_pestaña_persona(p_nombre):
    ...
    with col_f1:
        ...
        with st.form(f"form_v25_{p_id}", clear_on_submit=True):
            c_g1, c_g2 = st.columns(2)
            with c_g1:
                f_gasto = st.date_input("Fecha Pago", key=f"f_v25_{p_id}")
                concepto = st.text_input("Concepto / Detalle de Gasto", key=f"c_v25_{p_id}").strip()
                cat_g = st.selectbox("Categoría", CATEGORIAS_GASTO, key=f"cat_v25_{p_id}")
            with c_g2:
                ciudad_g = st.selectbox("Ubicación Geográfica", CIUDADES, key=f"ciudad_v25_{p_id}")
                monto_g = st.number_input("Monto total en pesos ($)", min_value=0.0, step=5000.0, format="%.0f", key=f"monto_v25_{p_id}")
    ...
    with col_f2:
        ...
        if not df_p.empty:
            ...
            gasto_origen = st.selectbox("1. Selecciona el gasto:", df_p["ID_Combo"].values, key=f"src_v25_{p_id}")
            opciones_destino = [dest for dest in INTEGRANTES if dest != p_nombre]
            persona_destino = st.selectbox("2. Enviar copia / mover a:", opciones_destino, key=f"dest_v25_{p_id}")
            
            c_btn1, c_btn2 = st.columns(2)
            with c_btn1:
                if st.button("👥 Copiar Gasto", key=f"btn_copy_v25_{p_id}", use_container_width=True):
                    df_global = pd.read_csv(GASTOS_FILE)
                    ...
                    if not fila_clonar.empty:
                        ...
                        st.rerun()
            with c_btn2:
                if st.button("➡️ Mover Gasto", key=f"btn_move_v25_{p_id}", use_container_width=True):
                    df_global = pd.read_csv(GASTOS_FILE)
    ...
    with col_eliminar:
        ...
        if not df_p.empty:
            df_p["ID_Combo"] = df_p["Concepto / Ítem"].astype(str) + " ($" + df_p["Costo ($)"].astype(str) + ")"
            gasto_baja = st.selectbox("Seleccione transacción a borrar:", df_p["ID_Combo"].values, key=f"baja_v25_{p_id}")
            if st.button("❌ Eliminar Transacción", type="primary", use_container_width=True, key=f"btn_baja_v25_{p_id}"):
                df_global = pd.read_csv(GASTOS_FILE)
    ...
    with col_tabla:
        st.markdown("##### 📋 Vista de Gastos")
        if not df_p.empty:
            df_mostrar_p = df_p[["Fecha", "Concepto / Ítem", "Categoría", "Ciudad", "Costo ($)"]].copy()
            # Mostramos de forma limpia sin llaves que generen conflictos reactivos en cascada
            st.dataframe(df_mostrar_p, use_container_width=True, hide_index=True)

# --- CONTROLADORES DE RENDERIZADO POR PÁGINA ---
if seleccion == "📊 Tablero y Finanzas":
    ...
    with col_izq:
        df_cal_editado = st.data_editor(df_cal_rutas, column_config=CONFIG_COLUMNA_RUTA, use_container_width=True, hide_index=True, key="ed_ruta_v25")
        if st.button("💾 Guardar Cambios de Calendario", type="primary", use_container_width=True):
    ...
    with c_al1:
        with st.form("form_alojamientos_v25", clear_on_submit=True):
    ...
    with c_al2:
        if not df_alojamientos.empty:
            al_borrar = st.selectbox("Selecciona para remover:", df_alojamientos["Nombre del Alojamiento"].values, key="del_aloj_v25")
            if st.button("❌ Eliminar Alojamiento", type="primary", use_container_width=True):
                ...
                st.rerun()
    if not df_alojamientos.empty: 
        st.data_editor(df_alojamientos, column_config=CONFIG_COLUMNA_ALOJAMIENTOS, use_container_width=True, hide_index=True, key="view_aloj_v25")

elif seleccion == "🗺️ Itinerario x Horas":
    st.title("🗺️ Cronograma de Actividades por Horas")
    col_add, col_del = st.columns([1.2, 1.8])
    with col_add:
        with st.form("form_itinerario_v25", clear_on_submit=True):
    ...
    with col_del:
        if not df_itinerario.empty:
            ...
            evento_borrar = st.selectbox("Selecciona hito a borrar:", df_itinerario["ID_Temp"].values, key="del_it_v25")
    ...
    if not df_itinerario.empty:
        ...
        st.data_editor(df_itinerario, column_config=CONFIG_COLUMNA_ITINERARIO, use_container_width=True, hide_index=True, key="view_it_v25")
    ...
    with col_ins:
        with st.form("form_nueva_tarea_v25", clear_on_submit=True):
    ...
    with col_rem:
        if not df_checklist.empty:
            tarea_a_borrar = st.selectbox("Selecciona qué quitar:", df_checklist["Tarea"].values, key="del_chk_v25")
            if st.button("❌ Borrar Definitivamente", type="primary", use_container_width=True):
                ...
                st.rerun()
    if not df_checklist.empty:
        df_checklist_editado = st.data_editor(df_checklist, column_config=CONFIG_COLUMNA_CHECKLIST, use_container_width=True, hide_index=True, key="ed_chk_v25")
